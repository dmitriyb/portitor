...
`portitor pr <fetch|comment|review|reply|resolve|describe|merge|close> --repo <name> --pr <n>` runs one action with portitor's credential after checking the caller's role against `action_roles` (default-deny). `review --inline` reads a JSON body raising inline threads; `reply --thread <id>` answers into a thread; `resolve --thread <id>` (or `--gate-threads` for every thread the gate's own reviews created) resolves them; `describe` overwrites the PR's body/description (modeled on `comment`, differing only in the gh call).
